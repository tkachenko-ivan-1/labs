...
class RationalList_Iterator:
    def __init__(self, elements):
        self._sorted_elements = sorted(
            elements,
            key=lambda item: (-item.get_d(), -item.get_n())
        )
        self._index = 0

    def __next__(self):
        if self._index < len(self._sorted_elements):
            result = self._sorted_elements[self._index]
            self._index += 1
            return result
        raise StopIteration
...
class RationalList:
    ...
    def __iter__(self):
        return RationalList_Iterator(self._elements)
...
def process_file(filename):
    ...
    try:
        ...
        print(f"Оригінальний порядок у списку:")
        print(r_list)

        print(f"\nОбхід через кастомний ітератор (сортування спадання d, потім n):")
        iterated_elements = []
        for item in r_list:
            iterated_elements.append(item)
        print(iterated_elements)
        print("-" * 50)
